...
while True:
    if running:
        ...
        for i,block in enumerate(lista_rects):
            ...
            if pg.Rect.collidepoint(bloco, ball_pos[0], ball_pos[1]) and ball_direction == UP_R:
                pontuação += lista_rects[i][2]
                lista_rects[i] = (0,0,0,0)
                ball_direction = DOWN_R
                print(pontuação)
            if pg.Rect.collidepoint(bloco, ball_pos[0], ball_pos[1]) and ball_direction == UP_L:
                pontuação += lista_rects[i][2]
                lista_rects[i] = (0,0,0,0)
